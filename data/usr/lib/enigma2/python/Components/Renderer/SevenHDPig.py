...
class SevenHDPig(Renderer):
	...
	def onShow(self):
		...
		if self.instance:
			if self.decoder>0:
				...
				if self.fb_size:
					SevenFBTool.setFBSize(self.fb_size,self.decoder)
				if self.PigStyle=="DualTV":
					SevenFBTool.setFBSize_delayed(self.fb_size2,decoder=0,delay=200)
			else:
				if self.Size:
	...
	def showpip(self):
		self.timer.stop()
		if config.plugins.SevenHD.PigMenuActive.value==False:
			InfoBar.instance.showPiP()
			InfoBar.instance.showPiP()
