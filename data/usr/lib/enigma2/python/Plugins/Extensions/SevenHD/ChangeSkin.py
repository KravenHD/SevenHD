...
class ChangeSkin():
        ...
        def do_update(self):
                self.skin_mode = config.plugins.SevenHD.skin_mode.value
                ...
                # 8K Skin 8192 x 4320
                   ...
                   self.debug('Y-Factor is set to: ' + str(self.y_factor))
                # User Selection
                elif self.skin_mode == '7':
                   x_res = config.plugins.SevenHD.skin_mode_x.value
                   y_res = config.plugins.SevenHD.skin_mode_y.value
                   self.x_factor = float(x_res) / float(tmp_skin_resolution[0])
                   self.y_factor = float(y_res) / float(tmp_skin_resolution[1])
                   resolution = '<resolution bpp="32" xres="' + str(x_res) + '" yres="' + str(y_res) +'" />'
                   #resolution = '<resolution bpp="32" xres="1280" yres="720" />'
                   self.debug('Resolution is set to: ' + resolution)
                   self.debug('X-Factor is set to: ' + str(self.x_factor))
                   self.debug('Y-Factor is set to: ' + str(self.y_factor))
