...
class SevenHDYWeather(Poll, Converter, object):
	city = 0
	direction = 1
	speed = 2
	humidity = 3
	...
	def get_xmlfile(self):
		self.iConsole.ePopen("wget -P /tmp -T2 'http://xml.weather.yahoo.com/forecastrss?w=%s&u=c' -O /tmp/SevenHDweather.xml" % config.plugins.SevenHD.weather_city.value, self.control_xml)
	...
	def getText(self):
		...
		if fileExists("/tmp/SevenHDweather.xml"):
			if int((time.time() - os.stat("/tmp/SevenHDweather.xml").st_mtime)/60) >= time_update:
				self.get_xmlfile()
		else:
			self.get_xmlfile()
			if not fileExists("/tmp/SevenHDweather.xml"):
				self.write_none()
				return 'N/A'
                if not fileExists("/tmp/SevenHDweather.xml"):
			self.write_none()
			return 'N/A'
		for line in open("/tmp/SevenHDweather.xml"):
